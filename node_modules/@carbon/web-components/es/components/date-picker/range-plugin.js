...
import rangePlugin$1 from 'flatpickr/dist/plugins/rangePlugin';
 ...
 */
var rangePlugin = (config) => {
    const factory = rangePlugin$1(Object.assign({ position: 'left' }, config));
    return (fp) => {
        ...
        const getDateStrFromInputs = (dates) => {
            return dates
                .filter((value) => value)
                .filter((d, i, arr) => fp.config.mode !== 'range' ||
                fp.config.enableTime ||
                arr.indexOf(d) === i)
                .join(fp.config.mode !== 'range'
        ...
        const handleBlur = (event) => {
            ...
            const valueChanged = getDateStrFromInputs([firstInput.value, secondInput.value]) !==
                fp.getDateStr();
            const relatedTargetIsCalendar = event.relatedTarget &&
                event.relatedTarget instanceof Node &&
                fp.calendarContainer.contains(event.relatedTarget);
            if (isInput && valueChanged && !relatedTargetIsCalendar) {
                fp.setDate([firstInput.value, secondInput.value], true, firstInput === fp.altInput
                    ? fp.config.altFormat
