    ...
    constructor() {
        ...
        this._handleBlur = async ({ target, relatedTarget }) => {
            ...
            if (!modal ||
                ...
                !modalContent ||
                !relatedTarget ||
                !modalContent.contains(relatedTarget)) {
                return;
            }
            const lastContent = modalContent.children[modalContent.children.length - 1];
            const gradientSpacing = modalContent.scrollHeight -
                lastContent.offsetTop -
                lastContent.clientHeight;
            for (let elem of modalContent.children) {
                if (elem.contains(relatedTarget)) {
                    const spaceBelow = modalContent.clientHeight -
                        elem.offsetTop +
